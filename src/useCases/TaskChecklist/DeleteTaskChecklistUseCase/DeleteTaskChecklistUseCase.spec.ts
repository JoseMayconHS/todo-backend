...
describe('Delete item in Checklist in Task', () => {
	...
	const deleteTaskChecklistUseCase = new DeleteTaskChecklistUseCase(
		taskChecklistRepository
	)

	it('should not be able delete item in checklist in task without id', async () => {
		let errorMessage = ''

		try {
			await deleteTaskChecklistUseCase.execute()
		} catch (e) {
	...
	it('should not be able delete item in checklist without task_id', async () => {
		let errorMessage = ''

		try {
			await deleteTaskChecklistUseCase.execute('123', '')
		} catch (e) {
			errorMessage = e.message
		}
	...
	it('should not be able delete item in checklist without workspace_id', async () => {
		let errorMessage = ''

		try {
			await deleteTaskChecklistUseCase.execute('123', '123')
		} catch (e) {
			errorMessage = e.message
		}
	...
	it('should not be able delete item in checklist without user_id', async () => {
		let errorMessage = ''

		try {
			await deleteTaskChecklistUseCase.execute('123', '123', '123')
		} catch (e) {
			errorMessage = e.message
		}
	...
	it('should be able delete item in checklist in task', async () => {
		...
		expect(
			tasks[0].checklist.some((checkitem) => checkitem._id === checkitem_id)
		).toBeTruthy()
		...
		expect(
			tasks_updated[0].checklist.some(
